...
Este proyecto implementa un pipeline de datos en Databricks para unificar tres sistemas de emplazamientos de red telco: **Centros TX**, **Sitios RAN** e **Infraestructura**.[file:21]
Históricamente cada módulo operaba por separado; aquí se construye un modelo único de emplazamientos que permite analizar estados, antigüedad de actualizaciones y disponibilidad por región y área operativa.[file:21]
...
- **Capa Bronze**: carga cruda de los Excels en tablas `raw_emplazamientos_*`, preservando al máximo la información original.[file:21]  
...
- **Capa Gold**: construcción de tablas analíticas:
  ...
  - Métricas de disponibilidad de emplazamientos por área (totales, disponibles y porcentaje disponible).[file:21]
...
Los datos representan emplazamientos de una compañía telco ficticia, exportados desde tres sistemas distintos (TX, RAN, Infraestructura) y consolidados en Databricks.[file:21]
...
- `notebooks/01_bronze_ingestion.py`  
  ...
  - Evita la conversión automática a `NaN` y crea tablas `raw_emplazamientos_centros_tx`, `raw_emplazamientos_infra` y `raw_emplazamientos_sitios_ran`.[file:21]
...
- `notebooks/02_silver_unification.py`  
  ...
  - Unifica los tres orígenes en una vista única de emplazamientos, permitiendo análisis consistentes entre módulos.[file:21]
...
- `notebooks/03_gold_analytics.py`  
  ...
  - Genera tablas agregadas mensuales por módulo, cabecera y `areaOYM`, y calcula ratios de disponibilidad de emplazamientos por área.[file:21]
...
- ¿Qué áreas (`areaOYM`) tienen menor **disponibilidad** de emplazamientos, y cómo ha evolucionado en el tiempo?[file:21]  
- ¿En qué regiones hay más emplazamientos con estados “baja”, “No Utilizado” o “desdefinido”, indicando posibles ineficiencias en el uso de la red?[file:21]  
- ¿Qué emplazamientos llevan más tiempo sin una actualización en los sistemas TX/RAN/Infraestructura, y deberían ser revisados o priorizados en futuras inversiones?[file:21]
...
3. Ajustar en `01_bronze_ingestion.py` la ruta base del volumen (`volume_path`) según tu entorno.[file:21]  
...
- Añadir un reporte en Power BI conectado a las tablas Gold, con páginas dedicadas a:
  ...
  - Fichas de emplazamiento con detalle TX/RAN/Infraestructura.[file:21]
